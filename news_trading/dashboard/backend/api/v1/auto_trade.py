...
async def start_auto_trade(
    ...
    Returns:
        Dict: 시작 결과
    """
    logger.info(f"[API] /auto-trade/start 요청 수신 (interval={interval})")
    service = get_auto_trade_service()

    # 설정 적용
    if config:
        logger.info("[API] 설정 적용 시작...")
        config_dict = config.model_dump()
        success = await service.update_config(config_dict)
        if not success:
            raise HTTPException(status_code=500, detail="설정 적용 실패")
        logger.info("[API] 설정 적용 완료")

    # 자동 매매 시작
    logger.info("[API] start_auto_trading 호출 시작...")
    success = await service.start_auto_trading(interval=interval)
    logger.info(f"[API] start_auto_trading 완료: success={success}")
